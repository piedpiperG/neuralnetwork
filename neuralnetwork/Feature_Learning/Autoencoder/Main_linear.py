...
# 训练函数
def train(model, data_loader, optimizer, criterion, device, l1_lambda=0.00005):
    model.train()
    total_loss = 0
    ...
    with torch.no_grad():
        for data, targets in data_loader:
            ...
            if criterion.__class__.__name__ == 'CrossEntropyLoss':
                ...
                targets = data
                outputs = outputs.view(64, 1, 28, 28)
            else:
                targets = data.view(data.size(0), -1)  # 展平目标，使之与输出形状相匹配
            loss = criterion(outputs, targets)
            total_loss += loss.item()
...
def test_and_evaluate(model, data_loader, criterion, device, num_images_to_display=10):
    ...
    with torch.no_grad():
        for data, targets in data_loader:
            ...
            if len(original_images) < num_images_to_display:
                ...
                reconstructed = outputs.view(outputs.size(0), 1, 28, 28).cpu().numpy()
    ...
    for i in range(num_images_to_display):
        ...
        axs[0, i].axis('off')
        axs[1, i].imshow(np.squeeze(reconstructed_images[i]), cmap='gray')
        axs[1, i].axis('off')
    plt.show()
...
# 加载模型
autoencoder_mnist = AdvancedAutoencoder(28 * 28).to(device)
...
# loss_types = ['mse', 'cross_entropy', 'mae', 'huber', 'ssim']
loss_types = ['mse']
