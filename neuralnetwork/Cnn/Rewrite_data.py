...
class Dropout(object):
    ...
    def forward(self, x):
        if self.train_mode:
            self.mask = np.random.binomial(1, 1 - self.p, size=x.shape) / (1.0 - self.p)
            return x * self.mask
        return x
...
class BatchNorm(object):
    ...
    def backward(self, delta):
        # Implementing backprop for batch normalization is a bit more involved
        # I'll skip it here for brevity, but you can find derivations and explanations online
        pass
